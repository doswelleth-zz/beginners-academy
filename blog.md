...
Bitcoin is open source software with a [well-documented process](https://bitcoin.org/en/developer-documentation){:target="_blank"} of how it arrives at its standards. Bitcoin is not a company. There is no marketing department, commercials, or holiday sales. Bitcoin does not have a slick PR guy or even a real consumer interface. And it is arriving slowly but surely into something for the general public.

At the moment, Bitcoin does generate free Bitcoin addresses, on or offline, for every man, woman and child, on planet earth. Deterministic software wallets that generate "seed tokens" with addresses allow users to backup and restore wallets from any device without knowing the exact number and letter sequence of their private keys. These recovery seeds further allow untrusted third-party systems to generate addresses without the ability to spend any bitcoins associated with these addresses.
